...
<?php  if (have_posts()) : $paged = (get_query_var('paged')) ? get_query_var('paged') : 1; query_posts( array('meta_key' => 'end_time','meta_compare' =>'>=','meta_value'=>time(),'meta_key' => 'imdbRating',
'post__not_in' => get_option( 'sticky_posts' ),'orderby' => 'meta_value_num', 'order' => 'DESC', 'paged' => $paged));
while ( have_posts() ) : the_post(); $imdbRating = get_post_meta($post->ID, "imdbRating", $single = true); get_template_part( 'item' ); endwhile; else : endif; ?>
</div>
<div id="pagination">
...
<option value="<?php echo esc_url( home_url( '/' ) ); ?><?php echo top; ?>/" selected='selected'>IMDb</option>
